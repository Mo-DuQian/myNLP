...
class TextLSTM(nn.Module):
    ...
    def LSTM(self, inputs, state=None):
        if state is None:
            hidden_state = torch.zeros(inputs.shape[1], n_hidden, device=device)  # [batch_size, n_hidden]
            cell_state = torch.zeros(inputs.shape[1], n_hidden, device=device)  # [batch_size, n_hidden]
...
def train_LSTMlm():
    ...
    for epoch in range(all_epoch):
        ...
        if (epoch + 1) % save_checkpoint_epoch == 0:
            torch.save(model, f'models/LSTMlm_simple_model_epoch{epoch + 1}.ckpt')
...
if __name__ == '__main__':
    ...
    print("\nTest the LSTMLM……………………")
    select_model_path = "models/LSTMlm_simple_model_epoch5.ckpt"
